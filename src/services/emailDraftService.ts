...
function extractWeeklyMailVisualStats(content: string) {
  ...
  return {
    ...
    riskCount: countWeeklyMailRowsInSection(content, "风险"),
  };
}
...
function weeklyMailVisualSummary(content: string) {
  ...
  const analysisBars = bars
    ...
    .join("");
  return `<div style="width:100%;max-width:100%;margin:12px 0 16px"><table role="presentation" cellspacing="0" cellpadding="0" style="width:100%;max-width:100%;border-collapse:collapse;table-layout:fixed"><tr><td style="width:33.333%;padding:0 4px 0 0;vertical-align:top"><div style="padding:10px 6px;border:1px solid #e5e7eb;border-radius:8px;background:#fbfdff;text-align:center;word-break:break-word;overflow-wrap:anywhere"><div style="display:inline-block;width:84px;height:84px;border-radius:999px;background:conic-gradient(#2563eb ${stats.progress}%, #e5e7eb 0);text-align:center;vertical-align:middle"><div style="display:inline-block;width:62px;height:62px;margin-top:11px;border-radius:999px;background:#ffffff;text-align:center"><div style="padding-top:16px;color:#111827;font-size:20px;font-weight:800;line-height:1">${stats.progress}%</div><div style="margin-top:3px;color:#6b7280;font-size:10px;font-weight:700;line-height:1">整体进度</div></div></div><div style="margin-top:9px;color:#6b7280;font-size:12px;line-height:1.45">已完成 <strong style="color:#111827;font-weight:800">${stats.doneCount}/${stats.totalCount}</strong> 项，开放 <strong style="color:#111827;font-weight:800">${stats.openCount}</strong> 项</div></div></td><td style="width:33.333%;padding:0 4px;vertical-align:top"><div style="padding:10px 6px;border:1px solid #e5e7eb;border-radius:8px;background:#fbfdff;text-align:center;word-break:break-word;overflow-wrap:anywhere"><div style="margin-bottom:10px;color:#6b7280;font-size:12px">项目状态</div>${weeklyMailStatusPill(stats.status)}<div style="margin-top:10px;color:#6b7280;font-size:12px;line-height:1.45">打开风险 / 问题 ${stats.riskCount} 项</div></div></td><td style="width:33.333%;padding:0 0 0 4px;vertical-align:top"><div style="padding:10px 6px;border:1px solid #e5e7eb;border-radius:8px;background:#fbfdff;word-break:break-word;overflow-wrap:anywhere"><div style="margin-bottom:8px;color:#6b7280;font-size:12px">本周分析</div><table role="presentation" cellspacing="0" cellpadding="0" style="width:100%;max-width:100%;border-collapse:collapse;table-layout:fixed">${analysisBars}</table></div></td></tr></table></div>`;
}
