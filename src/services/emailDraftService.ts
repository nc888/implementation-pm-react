...
function weeklyCustomerMailVisualSummary(content: string) {
  ...
  const barRows = bars
    .map(([label, value]) => {
      const width = Math.max(5, Math.round((value / totalCount) * 100));
      return `<tr><td style="width:62px;padding:4px 8px 4px 0;color:#64748b;font-size:12px;white-space:normal;word-break:break-word">${escapeHtml(label)}</td><td style="padding:4px 0"><span style="display:block;height:8px;overflow:hidden;border-radius:999px;background:#e8eef6"><span style="display:block;width:${width}%;height:100%;border-radius:999px;background:#0f766e"></span></span></td><td style="width:28px;padding:4px 0 4px 8px;color:#334155;font-size:12px;font-weight:800;text-align:right">${value}</td></tr>`;
    })
    .join("");
  ...
  return `<div style="width:100%;max-width:100%;margin:4px 0 18px"><table role="presentation" cellspacing="0" cellpadding="0" style="width:100%;max-width:100%;border-collapse:collapse;table-layout:fixed"><tr><td style="width:28%;padding:0 5px 0 0;vertical-align:top"><div style="${cardStyle};border-left:4px solid #0f766e;background:#f7fbfb"><div style="color:#64748b;font-size:12px;font-weight:700">客户协同看板</div><div style="margin-top:12px;color:#0f766e;font-size:25px;font-weight:800;line-height:1">${attentionLevel}</div><div style="margin-top:10px;color:#334155;font-size:12px;line-height:1.45">${attentionCount ? `${attentionCount} 项需要客户关注或配合` : "暂无需客户额外配合事项"}</div></div></td><td style="width:33%;padding:0 5px;vertical-align:top"><div style="${cardStyle};padding:0;overflow:hidden"><table role="presentation" cellspacing="0" cellpadding="0" style="width:100%;height:100%;border-collapse:collapse;table-layout:fixed"><tr><td style="padding:10px 6px;border-right:1px solid #edf2f7;text-align:center;vertical-align:middle"><div style="color:#64748b;font-size:12px">本周工作</div><div style="margin-top:9px;color:#172033;font-size:25px;font-weight:800">${workCount}</div></td><td style="padding:10px 6px;border-right:1px solid #edf2f7;text-align:center;vertical-align:middle"><div style="color:#64748b;font-size:12px">风险问题</div><div style="margin-top:9px;color:#172033;font-size:25px;font-weight:800">${riskIssueCount}</div></td><td style="padding:10px 6px;text-align:center;vertical-align:middle"><div style="color:#64748b;font-size:12px">下周计划</div><div style="margin-top:9px;color:#172033;font-size:25px;font-weight:800">${planCount}</div></td></tr></table></div></td><td style="width:39%;padding:0 0 0 5px;vertical-align:top"><div style="${cardStyle}"><div style="margin-bottom:8px;color:#64748b;font-size:12px;font-weight:700">内容分布</div><table role="presentation" cellspacing="0" cellpadding="0" style="width:100%;max-width:100%;border-collapse:collapse;table-layout:fixed">${barRows}</table></div></td></tr></table></div>`;
}
